...
discribe("Payments", function () {
    ...
    beforeEach(async function() {
        ...
        payments = await Payments.deploy() // 1Step- Send transaction
        await payments.deployed() //  2Step- wait for the transaction to complete
        console.log(payments.address)
    })

    it("should be deployed", async function() {
        expect(payments.address).to.be.properAddress
    }) //beforeEach is performed every time when we write it
    ...
    it("should be possible to send funds", async function() {
        ...
        await expect(() => tx)
            ...
            // .to.changeEtherBalances(acc2, -100)
        await tx.wait()
